...
def _validate_jdbba_bwd_inputs(
    jagged: torch.Tensor,
    dense: torch.Tensor,
    d_out: torch.Tensor,
    ...
    """Reject unsupported inputs before DLPack views / kernel launch."""
    ...
    _require_tensor("d_out", d_out, dtype=torch.bfloat16, ndim=2, device=device)
    _require_tensor(
...
def jagged_dense_bmm_bwd_dispatched(
    ...
    *,
    ...
    K, N, total_rows = _validate_jdbba_bwd_inputs(
        jagged, dense, d_out, seq_offsets, n_groups
    )
    D = int(K)
